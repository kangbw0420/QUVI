...
public class SemanticQueryExecutorNode implements WorkflowNode {
    ...
    @Override
    public String getId() {
        return "semantic_query_executor";
    }
    ...
    public void execute(WorkflowState state) {
        ...
        if (rawQuery == null || rawQuery.trim().isEmpty()) {
            log.error("SemanticQuery: SQL 쿼리가 비어있습니다.");
        ...
        try {
            log.info("=== SemanticQuery QueryExecutorNode 실행 시작 ===");
            log.info("SemanticQuery 원본 SQL 쿼리: {}", rawQuery);
            log.info("SemanticQuery LIMIT 값: {}", LIMIT);

            // SemanticQuery는 항상 일반 쿼리 처리 (API 분기 제거)
            ...
            // 5. 행 수 계산 및 페이지네이션
            String countResult;
            log.info("SemanticQuery COUNT 쿼리용 chainId 설정: {}", workflowId);
            DatabaseProfilerAspect.setWorkflowId(workflowId);
            countResult = queryRequest.countRows(viewQuery, LIMIT);
            log.info("SemanticQuery COUNT 쿼리 완료");

            log.info("SemanticQuery countRows API 응답: {}", countResult);

            int totalRows = 0;
            try {
                totalRows = Integer.parseInt(countResult);
                log.info("SemanticQuery 파싱된 총 행 수: {}", totalRows);
            } catch (NumberFormatException e) {
                log.warn("SemanticQuery 행 수 파싱 실패: {}", countResult);
                totalRows = 0;
            }
            state.setTotalRows(totalRows);

            if (totalRows > LIMIT) {
                log.info("✅ SemanticQuery 다음 페이지가 존재합니다. 총 행 수: {}, LIMIT: {}", totalRows, LIMIT);
                log.info("SemanticQuery 2단계: LIMIT 추가된 쿼리 생성 시작");
                state.setHasNext(true);
                viewQuery = queryRequest.addLimits(viewQuery, LIMIT, 0);
                log.info("SemanticQuery LIMIT 추가된 쿼리: {}", viewQuery);
            } else {
                state.setHasNext(false);
                log.info("❌ SemanticQuery 다음 페이지가 없습니다. 총 행 수: {}, LIMIT: {}", totalRows, LIMIT);
            }

            log.info("SemanticQuery 최종 hasNext 상태: {}", state.getHasNext());

            log.info("SemanticQuery 3단계: 실제 DB 쿼리 실행 시작");
            log.info("SemanticQuery 실행할 쿼리: {}", viewQuery);

            // 6. 쿼리 실행
            log.info("SemanticQuery 메인 쿼리용 chainId 설정: {}", workflowId);
            DatabaseProfilerAspect.setWorkflowId(workflowId);
            queryResult = mainJdbcTemplate.queryForList(viewQuery);
            log.info("SemanticQuery 메인 쿼리 완료, 반환 행 수: {}", queryResult.size());
            ...
            if (!queryResult.isEmpty()) {
                ...
                webSocketUtils.sendNodeEnd(state.getWebSocketSession(), "semantic_query_executor", data);
            }
            ...
            try {
                ...
                if ((queryResult.isEmpty() || queryUtils.isNullOnly(queryResult)) && !noteConditions.isEmpty()) {
                    ...
                    if (vectorNotes != null && !vectorNotes.isEmpty()) {
                        log.info("SemanticQuery: Found {} similar notes", vectorNotes.size());
                    ...
                    if (modifiedQuery != null && !modifiedQuery.equals(viewQuery)) {
                        log.info("SemanticQuery 수정된 쿼리용 chainId 설정: {}", workflowId);
                        DatabaseProfilerAspect.setWorkflowId(workflowId);
                        queryResult = mainJdbcTemplate.queryForList(modifiedQuery);
                        log.info("SemanticQuery 수정된 쿼리 완료, 반환 행 수: {}", queryResult.size());
                ...
                log.error("SemanticQuery note 조건 확인 중 오류 발생: {}", e.getMessage());
            }

            // 결과가 없는 경우 처리
            if (queryResult.isEmpty() || queryUtils.isNullOnly(queryResult)) {
                log.warn("SemanticQuery 쿼리 결과에 데이터가 없습니다");
            ...
            log.info("=== SemanticQuery QueryExecutorNode 실행 완료 ===");
            log.info("SemanticQuery 최종 상태 - hasNext: {}, totalRows: {}, queryResultSize: {}",
                    state.getHasNext(), state.getTotalRows(), queryResult.size());

        } catch (Exception e) {
            log.error("❌ SemanticQuery QueryExecutorNode 실행 중 예외 발생: {}", e.getMessage(), e);
            state.setQueryResultStatus("failed");
            state.setSqlError("SemanticQuery 쿼리 실행 실패: " + e.getMessage());
            state.setQueryError(true);
            state.setFinalAnswer(ErrorHandler.getWorkflowErrorMessage("SEMANTIC_QUERY_EXECUTION_ERROR", e.getMessage()));
        }
    }
    ...
    private List<String> extractColumnsFromSql(String sqlQuery) {
        ...
        try {
            ...
            if (normalizedSql.matches("(?i).*SELECT\\s+\\*\\s+FROM.*")) {
                log.debug("SemanticQuery SELECT * 쿼리이므로 결과에서 컬럼을 추출합니다.");
            ...
            java.util.regex.Matcher matcher = selectPattern.matcher(normalizedSql);
            if (matcher.find()) {
            ...
            log.warn("SemanticQuery SQL 파싱 중 오류 발생: {}", e.getMessage());
