...
def calculate_stats(result: List[Dict[str, Any]]) -> List[str]:
    ...
    Args:
        result: SQL 쿼리 결과의 row들을 담은 리스트
    ...
    # 통화 컬럼 찾기
    currency_column = find_currency_column(result)
    
    # 각 컬럼별 통계 계산
    for col in df.columns:
        # 각 컬럼 처리마다 result_parts 초기화
        result_parts = []
        
        # in_out_dv 컬럼은 입출금 통계에 사용되므로 개별 통계에서 제외
        if col in ['in_out_dv', 'view_dv', 'com_nm', 'acct_nick_nm', 'acct_dv', 'curr_cd']:
            continue
