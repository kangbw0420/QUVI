...
def final_df_format(result: Sequence[dict[str, Any]], selected_table: List[str]) -> List[Dict[str, Any]]:
    """선택된 테이블이 trsc를 포함하는 경우 데이터를 은행명과 계좌번호로 그룹화
    Returns:
        List[Dict[str, Any]]:
        - trsc가 포함되지 않은 경우: [{'data': result}]
        - trsc가 포함된 경우: 다음 형식으로 그룹화
          [
              {
                  'key': {
                      'title': '은행명',
    ...
    # trsc를 포함하지 않는 테이블이면 그대로 반환
    if 'trsc' not in selected_table:
        return [{'data': result}]
