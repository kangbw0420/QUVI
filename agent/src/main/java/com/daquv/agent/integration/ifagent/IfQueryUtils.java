...
//  import com.daquv.agent.integration.ifagent.IfQueryRequest;
...
//  @Component
//  @Slf4j
//  public class IfQueryUtils {

//  @Autowired
//  private IfExecutor ifExecutor;

//  @Autowired
//  private IfQueryRequest queryRequest;
...
//  companyConditions.add(String.format("%s.com_nm = '%s'", table.alias,
//  companyId));
//  }
...
//  // FROM 절 추출
//  Pattern fromPattern =
//  Pattern.compile("\\bFROM\\s+(\\w+)(?:\\s+(?:AS\\s+)?(\\w+))?",
//  Pattern.CASE_INSENSITIVE);
//  Matcher fromMatcher = fromPattern.matcher(query);
//  if (fromMatcher.find()) {
//  String tableName = fromMatcher.group(1);
//  String alias = fromMatcher.group(2) != null ? fromMatcher.group(2) :
//  tableName;

//  // SQL 키워드가 아닌지 확인
//  if (!sqlKeywords.contains(tableName.toUpperCase()) &&
//  !sqlKeywords.contains(alias.toUpperCase())) {
...
//  String alias = joinMatcher.group(2) != null ? joinMatcher.group(2) :
//  tableName;
...
//  // WHERE 절 찾기
//  Pattern wherePattern = Pattern.compile("\\bWHERE\\b",
//  Pattern.CASE_INSENSITIVE);
...
//  return query.substring(0, position) + " " + condition + " AND " +
//  query.substring(position).trim();
//  } else {
...
//  Pattern clausePattern = Pattern.compile("\\b" + clause + "\\b",
//  Pattern.CASE_INSENSITIVE);
...
//  return query.substring(0, insertPosition).trim() + " WHERE " + condition + " " + query.substring(insertPosition).trim();
//  } else {
//  // 끝에 WHERE 절 추가
...
//  try {
//  // ColumnRequest로 요청
//  return columnRequest.findColumnConditions(query, columnName).block();
//  } catch (Exception e) {
...
//  * 4. 쿼리 변환 (Python API)
//  */
//  public Map<String, Object> everNote(String query, UserInfo userInfo) {
//  try {
//  log.info("everNote 처리 시작: {}", query.substring(0, Math.min(query.length(),
//  100)));

//  // 1. note1 조건 추출 (ColumnRequest)
//  List<Map<String, Object>> noteConditions =
//  columnRequest.findNoteConditions(query).block();
...
//  // 2. 모든 노트 조회 (Java 구현)
//  List<String> availableNotes = getAllNotes(query, userInfo);
...
//  List<String> similarNotes = vectorRequest.getSimilarNotes(originalNote,
//  availableNotes, 10, 0.1);
//  allSimilarNotes.addAll(similarNotes);
//  }

//  // 4. 쿼리 변환 (ColumnRequest)
//  String modifiedQuery = columnRequest.transformQueryWithSimilarNotes(query,
//  noteConditions, allSimilarNotes)
//  .block();

//  log.info("everNote 처리 완료 - 유사 노트 개수: {}", allSimilarNotes.size());
//  return createEverNoteResult(modifiedQuery, originalNotes, new
//  ArrayList<>(allSimilarNotes));
...
//  */
//  private List<String> getAllNotes(String query, UserInfo userInfo) {
//  try {
...
//  // todo 유저 정보에서 가져와야함
//  List<Map<String, Object>> noteResults = ifExecutor.executeIf(noteQuery,
//  userInfo.getInttBizNo(),
//  userInfo.getInttCntrctId(), FETCH_ALL);
...
//  */
//  private Map<String, Object> createEverNoteResult(String query, List<String>
//  originNotes, List<String> vectorNotes) {
