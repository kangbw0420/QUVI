...
from graph.task.executor import executor
...
def make_graph() -> CompiledStateGraph:
    ...
    try:
        ...
        workflow.add_node("next_page", next_page)
        ...
        workflow.add_node("opendue", opendue)
        workflow.add_node("dater", dater)
        ...
        workflow.set_entry_point("next_page")

        workflow.add_conditional_edges(
            "next_page",
            lambda x: ("END" if x["user_question"] == "next_page" else "checkpoint"),
            {"END": END, "checkpoint": "checkpoint"},
        )
        workflow.add_conditional_edges(
            "checkpoint",
            lambda x: ("killjoy" if x["flags"]["is_joy"] else "isapi"),
            {"killjoy": "killjoy", "isapi": "isapi"},
        )
        workflow.add_conditional_edges(
            "isapi",
            lambda x: ("funk" if x["is_api"] else "commander"),
            {"funk": "funk", "commander": "commander"},
        )
        ...
        workflow.add_conditional_edges(
            ...
            lambda x: (
                "END"
                if x["flags"]["invalid_date"]
                else (
                    "yqmd"
                    if x["selected_api"] == "aicfo_get_financial_flow"
                    else "executor"
                )
            ),
            {"END": END, "yqmd": "yqmd", "executor": "executor"},
        )
        workflow.add_edge("yqmd", "executor")
        workflow.add_edge("commander", "opendue")

        workflow.add_conditional_edges(
            "opendue",
            lambda x: ("nl2sql" if x["flags"]["is_opendue"] else "dater"),
            {"nl2sql": "nl2sql", "dater": "dater"},
        )
        workflow.add_edge("dater", "nl2sql")
