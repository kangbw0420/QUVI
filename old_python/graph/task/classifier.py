...
async def classify_joy(query_text: str) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.info(
                f"Classification result: {results.get('checkpoint', 'unknown')}"
            )
            if logger.isEnabledFor(10):  # DEBUG level
                logger.info(
                    f"Full classification results: {json.dumps(results, indent=2, ensure_ascii=False)}"
                )
...
async def classify_api(query_text: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if logger.isEnabledFor(10):  # DEBUG level
                logger.info(
                    f"Full API classification results: {json.dumps(result, indent=2, ensure_ascii=False)}"
                )
...
async def classify_yqmd(query_text: str, sql_query: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if logger.isEnabledFor(10):  # DEBUG level
                logger.info(
                    f"Full YQMD classification results: {json.dumps(result, indent=2, ensure_ascii=False)}"
                )
            ...
            if sql_query.endswith(")"):
                ...
                logger.warning(
                    "Query doesn't end with ')', couldn't add YQMD parameter"
                )
...
async def classify_opendue(query_text: str) -> str:
    """개설일/만기일에 관한 질문인지 판단"""
    sanitized_query = sanitize_query(query_text)
    url = f"{BASE_URL}/opendue/{sanitized_query}"
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(url)
            response.raise_for_status()
            ...
            logger.info(f"Open date classification result: {result.get('opendue', '0')}")
            if logger.isEnabledFor(10):  # DEBUG level
                logger.info(
                    f"Full open date classification results: {json.dumps(result, indent=2, ensure_ascii=False)}"
                )

            return result.get("opendue", "0")

        except httpx.HTTPError as e:
            logger.error(f"Error classifying open date: {e}")
            return "0"
